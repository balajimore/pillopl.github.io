...
We already know that domain events are a great tool to integrate different part of your system (so called bounded contexts) and decouple them. Instead of directly calling outside component, we can notify our infrastructure, which in turns informs interested parties about what has just occured. This another form of <b>Inversion of Control</b> helps us to achieve our goal. Consider following piece of code:  
</p>
...
<p style="text-align:justify;">
There is a big problem with this simple <b>Observer</b> pattern in above example. Namely, everything happens in the same database transaction, which is started at the time of calling <i>register</i> method. This has several implications. First of all, we did not decouple user registration from e-mail sending as we tried to do with domain events. If our mail server is down, registration fails. Why would our user care about an email? He correctly filled registrtion form and does not even know there is an email coming. From the <b>use case</b> point of view, sending an email should not imply successful invocation. Secondly, even though it looks like the code follows deals only with users (it registers and marks as suspicious when needed), it modifies another aggregate - Communications (it would be probably modeled as a Generic Subdomain, but to simplify things consider it as another Bounded Context). The good rule of thumb says that we should not modify several aggregates in one transaction, because these concepts should not be so directly related. Sending an email (or any other action done as a consequence of registering user) may take a lot of time, do I/O calls, etc. But things get worse. Consider that everything was fine wit our mail server, so we came back from <i>DomainEventProcessor</i>, wrapped transaction ends and fails on database level while being committed. Now we are screwed, because confused user got error page as a response, but seconds later successful email with welcoming greetings.
</p>

<p style="text-align:justify;">
This cleary shows tat those concepts should be unrelated. One may come with an idea to fire all handlers asynchronously, but that solves only one issue: we could register users when we cannot send emails. We need something better. Fortunately, we can fire our emails just after transaction gets commited. Thus, we are sure everything was fine during registration and without doubts we can send the welcoming message. Spring gives us possibility to do that in a few lines of code with <a href="http://docs.spring.io/spring/docs/3.0.6.RELEASE_to_3.1.0.BUILD-SNAPSHOT/3.0.6.RELEASE/org/springframework/transaction/support/TransactionSynchronizationManager.html" TransactionSynchronizationManager</a>:
...
<p style="text-align:justify;">
Thanks to this code, we are fine with cause and effect relationship. Taking advantage of java memory model nomenclature: we know that successful registration <b>happens before</b> sending an email. One may argue that there is a slight window of time when we are not consistent. As stated at the begining, we don't need to be consistent when working with several bounded context. The imporant is that they will be <b>eventually consistent</b>. Anyway, we stil did not solve the problem with unresponsive mail server. When it fails, the information is lost. We can implement something which will retry this operation with a reasonable backoff, but when our application crashes, we still don't have any mean to recover to former state. We need to deal with that issue.
</p>
...
<p style="text-align:justify;">
We could call that component from our <i>handleNow</i> method from previous example and make some other component consume those messages and send emails. But that would be just delegeting the problem from mail server to another part of infrastructure - our queue. Putting a meesage to a queue might fail and the message would be lost. Moreover, our application can fail somewhere in between commit and invoking this asynchronous transaction listener. We also cannot move this code back to the transaction, because that would make us stuck at the beginning of our problem and queue failures would result in our users not being able to register. 

We have to develop a consistent solution in which an <b>occurence of an event reflects that it really happened in our system</b>. Also <b>when it really happened, it should be followed by an event</b>. In other words, those two statments should be in biconditional logical connective.

The problem is that our JMS component is not backed by the same data source as our domain model. One solution is to use global transaction and two-phase commits. The problem is that it might derease performance significantly. Plus, not every part of our infrastrucutre must support that mechanism. More clever idea is to share the same data source for our messaging infrastructure and domain model (if those two parts support that, of course). The side effect here is that we need to share the same schema, which might not be the nicest solution. In my opinion the best solution is to translate our events to our domain model storage and save them in the same transaction. Later on, another pool of threads can process them and publish somewhere else. Now our event store would look as follows:
</p>
...
<p style="text-align:justify;">
We mark every event as sent, so that it won't be picked up in further processing. If our message infracture fails, we try again soon. We have implemented a  We might argue that this code suffers from the same problem as the whole example. Our database might be done at the time we want to save this event as sent. That is fair concern, because we already have put it to JMS. That means it can arrive at consumer side several times. It is imporant that consumer handles those events in idempotent way by for example storing PersistentEvent's uuid and doing deduplication. Resending at producer side and idempotency at consumer side gives us <b>at most once delivery</b>
</p>
